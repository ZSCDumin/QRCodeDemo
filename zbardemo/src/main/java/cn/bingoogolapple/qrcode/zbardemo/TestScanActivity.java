...
//        mQRCodeView.startCamera(Camera.CameraInfo.CAMERA_FACING_FRONT);
    ...
    private static Bitmap getDecodeAbleBitmap(String picturePath) {
        try {
            ...
            options.inJustDecodeBounds = true;//表示只读图片，不加载到内存中，设置这个参数为ture，就不会给图片分配内存空间，但是可以获取到图片的大小等属性; 设置为false, 就是要加载这个图片.
            BitmapFactory.decodeFile(picturePath, options);
            int sampleSize = options.outHeight / 400;
            if (sampleSize <= 0) {
                sampleSize = 1;
            }
            options.inSampleSize = sampleSize;//设置恰当的inSampleSize可以使BitmapFactory分配更少的空间以消除该错误
